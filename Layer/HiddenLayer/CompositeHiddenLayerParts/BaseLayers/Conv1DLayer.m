classdef Conv1DLayer < CompositeBaseLayer & ParamsFunctions & ...
                       matlab.mixin.Copyable
   % A convolution layer for multiple channels of 1D signals.
   %
   % inputs should be 3D arrays with dimensions:
   % nChannels x batchSize x signalLength (C x N x X in the program comments)
   ...
   methods
      function obj = Conv1DLayer(inputSize, nChannels, filterSize, nFilters, ...
                                 varargin)
      ...
      function Wx = filter_activations(obj, x)
         ...
         for i = 1:(X - obj.filterSize + 1)
            xSeg = permute(x(:,:,i:i+obj.filterSize-1), [4, 2, 1, 3]); 
            % xSeg ~ 1 X N x C x fS
      ...
      function [grad, dLdx] = backprop(obj, x, dLdy)
         ...
         for i = 1:obj.filterSize
            xSeg = permute(x(:,:,i:ySize + i - 1), [4, 2, 1, 3]); 
            % xSeg ~ 1 x N x C x Size
            grad{1}(:,:,:,i) = mean(sum(bsxfun(@times, xSeg, ...
                              permute(dLdy, [1, 2, 4, 3])), 4), 2); % nF x 1 x C
         end
         
         [nF, N, ~] = size(dLdy);
         dLdx = obj.gpuState.zeros(obj.inputSize, N, obj.nChannels); 
         % dLdx ~ X x N x C (need to permute before returning)
         dLdyPadded = cat(4, obj.gpuState.zeros(nF, N, 1, obj.filterSize-1), ...
                              permute(dLdy, [1, 2, 4, 3]), ...
                              obj.gpuState.zeros(nF, N, 1, obj.filterSize-1)); 
         % dLdyPadded ~ nF x N x 1 x (X + fS - 1)
         
         for i = 1:obj.inputSize
            dLdySeg = dLdyPadded(:,:,:,i:i+obj.filterSize-1); % nF x N x 1 x fS
            dLdx(i,:,:) = sum(sum(bsxfun(@times, dLdySeg, ...
                                    flip(obj.params{1}, 4)), 4), 1); % 1 x N x C
         end
