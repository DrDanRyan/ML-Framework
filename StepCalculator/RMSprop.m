classdef RMSprop < StepCalculator
   % rmsprop (Hinton Coursera lectures) with NesterovMomentum. Like normal 
   % NesterovMomentum except gradients are divided by moving average of past
   % gradients prior to updating velocity. ParameterSchedule should provide 
   ...
   methods
      ...
      function take_step(obj, batch, model, params)
         ...
         if isempty(obj.velocity) % gradSquaredAvg should also be empty
            ...
            obj.gradSquaredAvg = cellfun(@(grad) grad.*grad, grad, ...
               'UniformOutput', false);
            if isscalar(lr) % lr and rho are scalars
               obj.velocity = cellfun(@(grad, avg) -lr*grad./...
                  (sqrt(avg)+obj.eps), grad, obj.gradSquaredAvg, ...
                  'UniformOutput', false);
            else % lr and rho are cell arrays
               obj.velocity = cellfun(@(lr, grad, avg) -lr*grad./...
                  (sqrt(avg)+obj.eps), lr, grad, obj.gradSquaredAvg, ...
                  'UniformOutput', false);
            end
            ...
            if isscalar(lr) % lr and rho are scalars
               ...
               obj.gradSquaredAvg = cellfun(@(avg, grad) obj.tau*avg + ...
                  (1-obj.tau)*grad.*grad, obj.gradSquaredAvg, grad, ...
                  'UniformOutput', false);
               obj.velocity = cellfun(@(vel, grad, avg) rho*vel - ...
                  lr*grad./(sqrt(avg)+obj.eps), obj.velocity, grad, ...
                  obj.gradSquaredAvg, 'UniformOutput', false);
               model.increment_params(cellfun(@(grad, avg) -lr*grad./...
                  (sqrt(avg)+obj.eps), grad, obj.gradSquaredAvg, ...
                  'UniformOutput', false));
               
            else % lr and rho are cell arrays
               model.increment_params(cellfun(@(rho, vel) rho*vel, rho, ...
               ...
               obj.velocity = cellfun(@(rho, vel, lr, grad, avg) rho*vel - ...
                  ...
                  obj.gradSquaredAvg, 'UniformOutput', false);
               model.increment_params(cellfun(@(lr, grad, avg) -lr*grad./...
                  (sqrt(avg)+obj.eps), lr, grad, obj.gradSquaredAvg, ...
                  'UniformOutput', false));
            end
         end
