classdef AdaDelta < StepCalculator
   % This class implements Zeiler2012 "AdaDelta: An adaptive learning rate
   % method". No ParameterSchedule is needed to use AdaDelta. There are two
   % hyperparameters, eps and tau, but the method is fairly robust to changes in
   % these two values (usually default is fine).
      ...
      function step = compute_step(obj, grad)
         if isempty(obj.gradSquared)
            obj.gradSquared = cellfun(@(grad) (1-obj.tau)*grad.*grad, grad, ...
                                      'UniformOutput', false);
            step = cellfun(@(gradAvg, grad) ...
                           -sqrt(obj.eps)./(sqrt(gradAvg)+obj.eps).*grad, ...
                           obj.gradSquared, grad, 'UniformOutput', false);
            obj.stepSquared = cellfun(@(step) (1-obj.tau)*step.*step, step, ...
                                      'UniformOutput', false);                                       
         else
            obj.gradSquared = cellfun(@(avg, grad) ...
                                 obj.tau*avg + (1-obj.tau)*grad.*grad, ...
                                 obj.gradSquared, grad, 'UniformOutput', false);
            ...
            obj.stepSquared = cellfun(@(avg, step) ...
                      obj.tau*avg + (1-obj.tau)*step.*step, ...
