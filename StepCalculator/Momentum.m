classdef Momentum < StepCalculator
   ...
   % corresponding ParameterSchedule provides params = {learnRate, momentum}.
   %
      ...
      function take_step(obj, batch, model, params)
         ...
         if isempty(obj.velocity)
            if isscalar(learnRate)
               obj.velocity = cellfun(@(grad) -learnRate*grad, grad, ...
                  'UniformOutput', false);
            else
            ...
            if isscalar(learnRate)
               obj.velocity = cellfun(@(grad, vel) momentum*vel - ...
                  learnRate*grad, grad, obj.velocity, 'UniformOutput', false);
            else
               obj.velocity = cellfun(@(lr, m, grad, vel) m*vel - ...
                  lr*grad, learnRate, momentum, grad, obj.velocity, ...
                  'UniformOutput', false);
            end
         end
